...
static void bleNotifySteeringAngle()
{
    ...
    { // ) && (char30.subscribed() -> dit bestaat precies niet voor esp32??
...
void setup()
{
    ...
    pinMode( PIO_RIGHT, INPUT );
    pinMode( PIO_LEFT, INPUT );

    BLEDevice::init( "steering-sterzo" );
    BLEServer *pServer = BLEDevice::createServer();
    pServer->setCallbacks(new MyServerCallbacks());
    BLEService *pSvcSterzo = pServer->createService( STERZO_SERVICE_UUID );
...
void loop()
{
    bool buttonLeft = digitalRead(PIO_RIGHT);
    bool buttonRight = digitalRead(PIO_LEFT);
    // TTP223 doesn't need debouncing

    if (!buttonLeft && !buttonRight)
    {
        buttonTaken = false; // listen for new key
    }
